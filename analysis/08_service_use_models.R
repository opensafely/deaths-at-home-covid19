...
model_emadm3_prop_cohort <- tibble(measure = c("emadm_1m", "emadm_3m", "emadm_1y")) %>%
  filter(!str_detect(measure, "^gp_hist")) %>%
  mutate(dataset = map(measure, function(var) df_input %>%
                         filter(!is.na(study_cohort)) %>% 
                         select(cohort, all_of(var)) %>%
...
gp_model_emadm3_prop_cohort <- tibble(measure = c("emadm_1m", "emadm_3m", "emadm_1y")) %>%
  mutate(activity = str_sub(measure, 1, -4)
         , period = str_sub(measure, -2, -1)
         , dataset = map2(activity, period, function(var, time) df_input %>%
                            filter(!is.na(study_cohort)) %>% 
...
gp_model_emadm3_prop_cohort_pod <- tibble(measure = c("emadm_1m", "emadm_3m", "emadm_1y")) %>%
  mutate(activity = str_sub(measure, 1, -4)
         , period = str_sub(measure, -2, -1)
         , dataset = map2(activity, period,  function(var, time) df_input %>%
                            filter(!is.na(study_cohort)) %>% 
